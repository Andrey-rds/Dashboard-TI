...
async function fetchDashboardData() {
  ...
  try {
    ...
    const cards = Array.isArray(data.cards) ? data.cards
      : (data.cards && Array.isArray(data.cards.card) ? data.cards.card : []);
    const tabelas = cards.filter(c => c && (c.tipo_card === 'tabela' || (c.cabecalho && c.linhas)));
    ...
    if (Array.isArray(data.kpis)) {
      data.kpis.forEach((kpi, index) => {
        ...
        if (elValor) {
          ...
          if (kpi.cor_texto_valor) elValor.style.color = kpi.cor_texto_valor;
        }

        if (elTituloKpi) {
          elTituloKpi.innerText = kpi.titulo ?? '';
          if (kpi.cor_texto_titulo) elTituloKpi.style.color = kpi.cor_texto_titulo;
        }
      });
    }
      ...
      if (configEsq.cabecalho) {
        ...
        if (trCabecalhoEsq) {
          trCabecalhoEsq.innerHTML = Object.values(configEsq.cabecalho)
            .map(col => `<th class="py-2 px-2 font-bold text-sm text-slate-400 uppercase tracking-wider">${col}</th>`)
            .join('');
        }
      }
      ...
      if (tbodyEsq && Array.isArray(configEsq.linhas)) {
        tbodyEsq.innerHTML = configEsq.linhas.map(linha => `
          <tr class="hover:bg-slate-800/40 transition-colors">
            ...
            <td class="py-2 px-2 text-base font-bold text-slate-100 uppercase">${linha.celula2 ?? ''}</td>
            <td class="py-2 px-2 text-sm">
              <span class="px-2 py-0.5 rounded text-sm font-black bg-cyan-500/20 text-cyan-300 border border-cyan-500/40 whitespace-nowrap inline-block text-center uppercase tracking-wider">
                ${linha.celula3 ?? ''}
              </span>
            </td>
            <td class="py-2 px-2 text-base font-semibold text-slate-300 uppercase">${linha.celula4 ?? ''}</td>
            <td class="py-2 px-2 text-base font-bold text-slate-200 uppercase">${linha.celula5 ?? ''}</td>
            <td class="py-2 px-2 text-base font-medium text-slate-300 uppercase">${linha.celula6 ?? ''}</td>
            <td class="py-2 px-2 text-base font-medium text-slate-200 uppercase overflow-hidden whitespace-nowrap max-w-[200px]">${linha.celula7 ?? ''}</td>
          </tr>
        `).join('');
      }
    }
      ...
      if (tbodyDir && Array.isArray(configDir.linhas)) {
        tbodyDir.innerHTML = configDir.linhas.map(linha => `
          <tr class="hover:bg-slate-800/40 transition-colors">
            ...
            <td class="py-2 px-2 text-base font-bold text-slate-100 uppercase">${linha.celula2 ?? ''}</td>
            <td class="py-2 px-2 text-sm">
              <span class="px-2 py-0.5 rounded text-sm font-black bg-amber-500/20 text-amber-300 border border-amber-500/40 whitespace-nowrap inline-block text-center uppercase tracking-wider">
                ${linha.celula3 ?? ''}
              </span>
            </td>
            <td class="py-2 px-2 text-base font-semibold text-slate-300 uppercase">${linha.celula4 ?? ''}</td>
            <td class="py-2 px-2 text-base font-medium text-slate-300 uppercase">${linha.celula5 ?? ''}</td>
            <td class="py-2 px-2 text-base font-medium text-slate-300 uppercase">${linha.celula6 ?? ''}</td>
          </tr>
        `).join('');
      }
