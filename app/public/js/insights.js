...
function cardsDeInsights(insights) {
  ...
  // indicadores: [{ indicador, valor, impacto }]
  if (Array.isArray(insights.indicadores) && insights.indicadores.length) {
    cards.push({
      tipo: 'destaque',
      titulo: 'Indicadores',
      itens: insights.indicadores.map(i =>
        `${i.indicador ?? ''}: ${i.valor ?? ''}${i.impacto ? ` (impacto ${i.impacto})` : ''}`
      )
    });
  }
...
function renderizarInsights(insights) {
  ...
  insights.forEach(item => {
    ...
    card.className = 'insight-card bg-[#131927] border border-gray-800/80 rounded-lg p-3.5 flex flex-col shadow-md min-h-0 overflow-hidden';
    ...
    card.innerHTML = `
      <div class="flex items-center gap-2 mb-2 shrink-0">
        ...
        <span class="text-[9px] uppercase font-bold tracking-wider" style="color:${cor};">${cfg.rotulo}</span>
      </div>
      <h3 class="text-sm md:text-base font-bold text-white leading-tight mb-1.5">${escaparHtml(item.titulo)}</h3>
      ${Array.isArray(item.itens) && item.itens.length
        ? `<ul class="text-xs md:text-sm text-gray-400 leading-snug overflow-hidden list-disc pl-4 space-y-0.5">
             ${item.itens.map(t => `<li>${escaparHtml(t)}</li>`).join('')}
           </ul>`
        : `<p class="text-xs md:text-sm text-gray-400 leading-snug overflow-hidden">${escaparHtml(item.descricao)}</p>`}
    `;
